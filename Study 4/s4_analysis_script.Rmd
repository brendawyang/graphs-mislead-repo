...
`r text_spec(round(sum(education[education$dem_ed >= 5,]$percentage)), bold = T)`% reported having at least a Bachelor’s degree.  The samples also exhibited a range of graph literacy (see Table 1).
...
ggplot(subject_mean_df, aes(x = session, y = subject_mean_rating, fill = graph_condition)) +
  ...
  ylab("mean rating") +
  xlab("session") +
  ggtitle("Figure 7. study 4 results")
...
model_full <- lmer(rating ~ subject_condition * graph_condition * session + (1 | participantid), models_df) # CI bootstrapping doesn't converge item is added as a random effect
...
We did not find evidence for our hypothesis that the protective effect of an explanatory warning would fade by the second session. The 3-way interaction between timepoint, warning condition, and graph type was not statistically significant (t = 1.43, p = .15). A model comparison between the linear effects model described above and one that did not include timepoint and its associated interactions did not reveal statistically significant differences i.e., the main effect of timepoint was not significant. However, Figure 7 and statistically significant 2-way interactions from this linear model suggest that timepoint may have affected participant ratings in unanticipated ways. However, our results were not definitive, and it is possible we are under-powered to fully examine these smaller effects. We report post-hoc analyses relating to the effects of timepoint in Supplemental Information due to their exploratory nature.
...
# Previous submission
The analyses below were removed for a manuscript revision.
