...
rating_df <- read_csv("data/clean/s5_rating_df.csv", col_types = cols(field = col_factor(levels = c("Humanities", "Quantitative")), graph_condition = col_factor(levels = c("control", "truncated")), subject_condition = col_factor(levels = c("no warning", "warning"))))
rating_df$field <- tolower(rating_df$field)
rating_df$field <- as.factor(rating_df$field)
...
model_full<- lmer(rating ~ subject_condition*graph_condition*field + (1 | participantID) * (1 | question), models_df)
...
ci_model_full <- confint(model_full, "beta_", level = 0.95, method = c("boot"), nsim = 1000,  seed = 833, boot.type = c("perc"), FUN=NULL, quiet=FALSE, oldNames = FALSE, .progress = "txt")
...
thetable_full[numVars] <- lapply(thetable_full[numVars], round, digits = 2) 
...
emmip(model_full, field ~ subject_condition) # interaction
emmip(model_full, graph_condition ~ field) # maybe

# 3-ways
#emmip(model_full, graph_condition ~ field | subject_condition)
#emmip(model_full, graph_condition ~ subject_condition | field )
#emmip(model_full, subject_condition ~ field | graph_condition)
#emmip(model_full, subject_condition ~  graph_condition | field)
#emmip(model_full, field ~ subject_condition | graph_condition)
#emmip(model_full, field ~ graph_condition | subject_condition)
...
# graph type x field
s_g <- emmeans(model_full, pairwise ~ field | graph_condition, lmer.df = "satterthwaite")
s_g
confint(s_g$contrasts)

g_s <- emmeans(model_full, pairwise ~ graph_condition | field, lmer.df = "satterthwaite")
g_s
confint(g_s)

compare <- pairs(pairs(emmeans(model_full, ~ graph_condition|field)), by = NULL)
compare
confint(compare)


# field x warning
f_s <- emmeans(model_full, pairwise ~ field | subject_condition, lmer.df = "satterthwaite")
f_s
confint(f_s)

s_f <- emmeans(model_full, pairwise ~ subject_condition | field, lmer.df = "satterthwaite")
s_f
confint(s_f)

compare <- pairs(pairs(emmeans(model_full, ~ field|subject_condition)), by = NULL)
compare
confint(compare)
...
errbar_lims <- subject_mean_df %>% 
  group_by(graph_condition, subject_condition, field) %>% 
...
```{r figure 8 prep, eval=FALSE, include=FALSE}
# # Get the CIs
# 
# # estimated marginal means pair-wise comparisons for all combinations of IVs
# emm1 = emmeans(model_full, specs = pairwise ~ graph_condition + field + subject_condition, lmer.df = "satterthwaite")
...
#   summarise(mean=mean(subject_mean_rating),
#             se=sd(subject_mean_rating)/sqrt(n()), 
#             upperCI=mean+(2*se), lowerCI=mean-(2*se)) %>%
#   left_join(emm1.df) # add in more precise CIs
# 
# subject_mean_df %<>% 
#   mutate(subject_condition_2 =ifelse(subject_condition == "no warning", 1, 2))
...
ggplot(subject_mean_df, aes(x = as.character(field), y = subject_mean_rating, fill = graph_condition)) +
  ...
  raincloud_theme +
  ggtitle("Study 5. c- and d- adjusted CIs")
...
control,quantitative,warning - truncated,quantitative,warning
estimate= -0.22939307
SE = 0.03439020
z.ratio = -6.6703037
p = 0.00000000071444516969
...
t.test(subject_mean_rating ~ graph_condition, df, paired = TRUE, var.equal= TRUE)
